...
void Writing::wyswietl(float u1, float v1, float u2, float v2, float x1, float y1, float x2, float y2)
{
    ...
    buffer.vertex(x1, y1, 0, 0, 1, u1, v1);
    buffer.vertex(x2, y2, 0, 1, 0, u2, v2);
    buffer.vertex(x1, y2, 1, 0, 0, u1, v2);

    buffer.vertex(x1, y1, 0, 0, 1, u1, v1);
    buffer.vertex(x2, y1, 1, 0, 0, u2, v1);
    buffer.vertex(x2, y2, 0, 1, 0, u2, v2);

//    buffer.vertex(x1, y1, 0, 0, 1, 0.0625, 0);
//    buffer.vertex(x2, y2, 0, 1, 0, 0.09375, 0.166666666);
//    buffer.vertex(x1, y2, 1, 0, 0, 0.0625, 0.166666666);
//
//    buffer.vertex(x1, y1, 0, 0, 1, 0.0625, 0);
//    buffer.vertex(x2, y1, 1, 0, 0, 0.09375, 0);
//    buffer.vertex(x2, y2, 0, 1, 0, 0.09375, 0.166666666);
...
void Writing::pisz(string text)
{
    ...
    if(pos==CENTER)
    {
        horizontal_position=-1*(text.size()*width*0.5);
    }
    if(pos==LEFT)
    {
        horizontal_position=-1+margin_left;
    }
    float y1=1-(margin_top+vertical_position);
    float y2=y1-height;
...
Writing::Writing(float height_, float width_, float margin_top_, string font_file_name):font(font_file_name)
{
    vertical_position=0;
